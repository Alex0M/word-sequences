...
This version implemented the approach to read a file by chunks concurrently using some number of workers.

Our pipeline for workers will have next steps:  
Worker #1: [read chunks of file] -> [procesing: split chunks to word sequences] -> [calculate word sequences putting to a map #1]  
Worker #2: [read chunks of file] -> [procesing: split chunks to word sequences] -> [calculate word sequences putting to a map #2]  
Worker #N: [read chunks of file] -> [procesing: split chunks to word sequences] -> [calculate word sequences putting to a map #N]  
